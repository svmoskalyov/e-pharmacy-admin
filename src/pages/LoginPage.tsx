...
function LoginPage() {
  ...
  const createErrorMessage = (error: string) => {
    if (`${error}` === 'Firebase: Error (auth/too-many-requests).') {
      setErrorAuth('Too many requests.')
    } else if (
      `${error}` === 'Firebase: Error (auth/network-request-failed).'
    ) {
      setErrorAuth('Problem with network')
    } else if (
      `${error}` === 'Firebase: Error (auth/invalid-login-credentials).'
    ) {
      setErrorAuth('Email or password is not correct')
  ...
  return (
    <>
      ...
      {error && <Alerti severity="error" message={errorAuth} />}
    </>
  )
}
